...
void MapUI::PrintMenu() {
  ...
  case 5:
  {
    ...
    std::cout << menu << std::endl;
    // fill in here
    // menu = "Please input the expression:";
    // std::cout << menu;
    // std::regex exp;
    // getline(std::cin, exp);
    // auto output = map.GetLocationRegex(exp);
    // for (auto x: output) {
    //   std::cout << "\""  << x << "\"" << ", ";
    // }
    // PlotPoints(output);
  ...
  case 12:
  {
    ...
    std::cout << menu;
    if (results.size() != 0) {
      for (unsigned int i = 0; i < Q.size(); i++) std::cout << "From " << Q[i].second[0] << " to " << Q[i].second[1] << " with " << Q[i].first << " gallons of gas tank: " << (results[i] ? "Yes" : "No") << std::endl;
    } 
    menu = "**************************************************************\n";
    std::cout << menu;
    std::cout << "Time taken by function: " << duration.count()/1000 << " ms" << std::endl << std::endl;
